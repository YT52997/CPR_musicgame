...
def score_DFT(A,f):
    ...
    # 考慮範圍 0<--[100bpm,120bpm]-->2
        ...
        # 假設一般人做cpr最快只會壓到4赫茲，因此max_shift = 2
            ...
            shift = f[max_idx]-2.0
            max_shift = 2
        # 假設一般人做cpr最慢只會壓到1.5赫茲，因此max_shift = 1.5
        else:
...
"""
# 載入音效
#sound_effect = pygame.mixer.Sound(os.path.join("audio","Baby shark.mp3"))
"""
factor = 5
# 顯示畫面
WIDTH = 408
...
class sensorData(pygame.sprite.Sprite):
    ...
    def get_rank(self):
        ...
        # 若讀值過小，則頻率和位移分數為零，回傳頻率和位移值為-1，提示訊息為No movement
        # deltax為要回傳的位移值
        if delta_x<0:
            self.freqRank="No movement!!"
            self.displacementRank = "No movement!!"
        ...
        # 計算位移值偏離5公分的比率，換成百分比
        shift_rate = delta_x-5.0
        shift_rate /= 5
        shift_rate *= 100
        deltax_score = 100 - abs(shift_rate)
        # deltxa為要回傳的位移值
        deltax=delta_x

        # 位移誤差容忍20%以內
        if abs(shift_rate) <= 20:
            self.displacementRank = "keep this depth"
            deltax_score = 100
        elif shift_rate<0:
            self.displacementRank = "Press deeper!!"
        else:
            self.displacementRank = "Press shallower!!"
    ...
    def update(self):
        ...
        # 先將頻率轉為BPM、位移四捨五入至小數第二位
        self.freq_msg=str(int(freq*60))
        self.deltax_msg=str(round(deltax,2))
        ...
        # 若位移落在標準範圍中，用綠字顯示"keep this depth"
